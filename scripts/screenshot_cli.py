...
def emit_path(path: Path, *, clipboard: bool = True) -> None:
    ...
    What's *printed* is always the bare path, quoted nowhere: terminals
    detect a bare `/mnt/c/...` and make it ctrl-clickable, and surrounding
    quotes break that detection -- which costs more than it buys, since
    opening the image is the whole point. The spaces problem is solved on
    the clipboard instead, which gets the shell-quoted form ready to paste
    into a command.
    """
    if sys.stdout.isatty() and clipboard:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs='?',
        default='take',
        choices=('take', 'latest', 'list', 'dir', 'move', 'open'),
