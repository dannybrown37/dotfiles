...
5. Implement per the repo's standing conventions (see `.claude/CLAUDE.md`): TDD, discrete reviewable steps, invoke the relevant language skill (`bash`, `node`, `python`, `dotfiles`, `gtd`) for the code being touched. Never `git add`/`commit`/`push` — that stays manual.
