...
`scripts/verify_changes.py` (Stop hook) already gates lint and tests on changed files. This
...
Drawn from this repo's own session history:

- Calling a rendering fix done without ever rendering it — cost repeated rounds of "still not
  rendering correctly" and "still getting the ? in a square".
- Answering "is the 12-week year eliminated?" from the one file that had just been edited,
  leaving the user to ask "can you confirm that?".
- Reporting work as done while `ruff`, `pytest`, or CI would have failed on it.
