...
git write. Same human-in-the-loop boundary as the rest of this repo — this skill invokes
`gh`, not `git commit`/`git push`.
...
5. **Fix it following this repo's standing rules**: TDD (test first where the failure is a
   code bug), discrete steps, invoke the relevant language skill/reference
   (`.claude/references/*-style.md`) for the code being touched.

6. **Verify locally before reporting done** — invoke `verify`. Run the same check CI ran
   (test suite, pre-commit, lint) so "should pass now" is backed by a real local run, not
...
7. **Stop. Show the diff. Do not `git add`/`commit`/`push`.** Report what was wrong, what
   ...
   the re-run is manual, same as every other change in this repo.
